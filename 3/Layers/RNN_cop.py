...
class RNN(BaseLayer):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self._optimizer.add_regularizer(Constraints.L2_Regularizer(0.1))
    ...
    def forward(self, input_tensor):
        ...
        FC1_1 = FullyConnected(input_tensor.shape[1] + self.hidden_state.size , self.hidden_state.size)
        TanH_FC1_1 = TanH()
        FC2_1 = FullyConnected(self.hidden_state.size, self.output_size)
        Sigmoid_FC2_1 = Sigmoid()

        if self._weights is None:
            self._weights = FC1_1.weights
        else:
            FC1_1.weights = self._weights

        if self.weights_initializer is None:
            self.FC2_weights = FC2_1.weights
        else:
        ...
        self.layers.extend((FC1_1, TanH_FC1_1, FC2_1, Sigmoid_FC2_1))
        ...
        for i in range(input_tensor.shape[0]):
            ...
            FC1 = deepcopy(FC1_1)
            TanH_FC1 = deepcopy(TanH_FC1_1)
            FC2 = deepcopy(FC2_1)
            Sigmoid_FC2 = deepcopy(Sigmoid_FC2_1)


            test = FC1.forward(x_tilde)
            self.hidden_state = TanH_FC1.forward(FC1.forward(x_tilde))
            test = FC2.forward(self.hidden_state)

            output_tensor[i] = Sigmoid_FC2.forward(FC2.forward(self.hidden_state))
    ...
    def backward(self, error_tensor):
        ...
        for i in range(error_tensor.shape[0]):
            ...
            Sigmoid_FC2 = self.layers[index]
            FC2 = self.layers[index - 1]
            error_gradient = Sigmoid_FC2.backward(error_tensor[i])
            error_gradient = FC2.backward(error_gradient)
            # error_gradient = FC2.backward(Sigmoid_FC2.backward(error_tensor[i]))
            if i > 0:
                error_gradient += error_hidden

            TanH_FC1 = self.layers[index - 2]
            FC1 = self.layers[index - 3]
            error_gradient = TanH_FC1.backward(error_gradient)
            error_gradient = FC1.backward(error_gradient)
